...
def ref_to_id(ref: str | None) -> str | None:
    """
    Local id from a FHIR Reference string.

    Handles ``Patient/abc``, absolute URLs, and ``urn:uuid:...`` (common in document Bundles).
    """
    if not ref:
        return None
    s = str(ref).strip()
    if not s:
        return None
    sl = s.lower()
    if sl.startswith("urn:uuid:"):
        return s[9:].strip()
    for marker in ("/Patient/", "/Encounter/", "/Practitioner/", "/Organization/"):
        if marker in s:
...
def encounter_status_code(encounter: dict[str, Any] | None) -> str:
    """
    FHIR ``Encounter.status`` as a lowercase code string (e.g. ``finished``, ``in-progress``).

    Handles normal JSON (string) and rare ``CodeableConcept``-shaped values.
    """
    if not isinstance(encounter, dict):
        return ""
    s = encounter.get("status")
    if isinstance(s, str):
...
def _extract_encounter_from_subscription_status(st: dict[str, Any]) -> dict[str, Any] | None:
    """
    Medplum/R5 subscription-notification Bundles often put ``SubscriptionStatus`` first and may
    embed the changed ``Encounter`` inside ``notificationEvent[].focus`` (not only as entry[1]).
    """
    for ne in st.get("notificationEvent") or []:
        if not isinstance(ne, dict):
            continue
        focus = ne.get("focus")
        if isinstance(focus, dict) and str(focus.get("resourceType") or "") == "Encounter":
            return focus
    return None
...
def extract_encounter_and_bundle(webhook_data: dict[str, Any] | None) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
    ...
    - Bare ``Encounter``
    - ``Bundle`` (Medplum rest-hook: ``type`` often ``history``) — scans **all** ``entry`` for
      ``Encounter``, then checks ``SubscriptionStatus`` for embedded Encounter in ``focus``
    - Wrapper ``{ "resource": { "resourceType": "Encounter", ... } }``
    """
    ...
    if rtype == "Bundle":
        entries = _bundle_entries(webhook_data)
        encounters = [r for r in entries if str(r.get("resourceType") or "") == "Encounter"]
        enc = None
        if encounters:
            # Prefer the Encounter in ``finished`` state (subscriptions often include multiple versions).
            for e in encounters:
                if encounter_status_code(e) == "finished":
                    enc = e
                    break
            if enc is None:
                enc = encounters[-1]
        if enc is None:
            for res in entries:
                if str(res.get("resourceType") or "") == "SubscriptionStatus":
                    enc = _extract_encounter_from_subscription_status(res)
                    if enc:
                        break
    ...
    if isinstance(fu, dict):
        ...
        if fu.get("days") is not None:
            try:
                ...
                pass
        # Case-sheet JSON often sends days without explicit required — treat as follow-up needed.
        if "required" not in fu and cur.get("days") is not None:
            try:
                if int(cur["days"]) > 0:
                    cur["required"] = True
            except (TypeError, ValueError):
